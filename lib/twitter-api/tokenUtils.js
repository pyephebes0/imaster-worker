...
export async function refreshTokenIfNeeded(account) {
  ...
  console.log(
    `🔍 เงื่อนไข: เวลาปัจจุบัน < วันหมดอายุ - 60 วินาที ?`,
    now < account.expiresAt?.getTime() - 60_000
  );

  // เช็คว่า token ยังไม่หมดอายุ (เหลือเกิน 60 วินาที)
  if (account.expiresAt && now < account.expiresAt.getTime() - 60_000) {
    console.log(`✅ Token ยังใช้ได้ ไม่ต้องรีเฟรช`);
    return account;
  }
